// EMF Machine : 
// elliott903
function emfelliott903(options) {
  ...
  m.bus.display = elliott903_display(m.bus, options);
  m.bus.cpu = {};
  m.bus.cpu.name = "cpu";
  m.bus.cpu.emulate = elliott903_cpu_emulator(m.bus, options);
  m.bus.cpu.disassemble = elliott903_cpu_disassemble(m.bus, options);
  m.bus.memory = elliott903_memory(m.bus, options);

  m.bus.clock = {};
  m.clock = {};
  m.clock.cpu = elliott903_clock_cpu(m, options);
  m.bus.clock.cpu = m.clock.cpu;


  m.reset = function() {
    if (m.bus.cpu.emulate.reset) m.bus.cpu.emulate.reset();
    if (m.bus.memory.reset) m.bus.memory.reset();
    if (m.bus.display.reset) m.bus.display.reset();
    //if (m.bus.iorq.reset)  m.bus.iorq.reset();
    if (m.clock.cpu.cpu) m.clock.reset.reset();
  };

  m.update = function(how) {
    let processed = {};
    processed.cpu = m.bus.cpu.emulate.update(how);
    return processed;
  };

  m.start = function() {
    if (m.bus.cpu.emulate.start) m.bus.cpu.emulate.start();
    if (m.bus.memory.start) m.bus.memory.start();
    if (m.bus.display.start) m.bus.display.start();
    //if (m.bus.iorq.start)  m.bus.iorq.start();
    if (m.clock.cpu.cpu) m.clock.start.start();
  };
  /*
  m.bus.iorq = elliott903_iorq(options, m.bus);
  */
  m.reset();

  m.options = {};
  m.options.disassemble = {};
  m.options.disassemble.widthColumnHex = 0
  m.options.disassemble.widthColumnInstruction = 10
...
Elliott = function() {
  ...
  function reset() {
    ...
    if (device_list.plotter && device_list.plotter.reset) {
      device_list.plotter.reset();
    }
    if (device_list.tape1 && device_list.tape1.reset) {
      device_list.tape1.reset();
    }
    if (device_list.tape2 && device_list.tape2.reset) {
      device_list.tape2.reset();
    }
    if (device_list.teleprinter1 && device_list.teleprinter1.reset) {
      device_list.teleprinter1.reset();
    }
    if (device_list.teleprinter2 && device_list.teleprinter2.reset) {
      device_list.teleprinter2.reset();
    }
  }
  ...
  function assemble(addr, text, state) {
    ...
    return {
      ...
      error: value === undefined ? "Could not assembler" : null
    };
  }
